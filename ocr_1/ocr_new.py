...
class WatsonxOCRExtractor:
    ...
    def resize_image_for_display(self, image, max_width=600):
        """Resize image to fit Word document width while maintaining aspect ratio"""
    ...
    def pdf_page_to_image(self, pdf_document, page_number):
        ...
        # Render page to an image with high resolution (300 DPI equivalent)
    ...
    def process_pdf(self, pdf_file, start_page, end_page, range_label="Range"):
        ...
        try:
            ...
            if start_page > total_pages or end_page > total_pages:
                st.error(f"Invalid page range. PDF has only {total_pages} pages.")
                return "Error: invalid page range", [], []
            
            # Convert pages to images
            images = []
            for page_num in range(start_page, end_page + 1):
                image = self.pdf_page_to_image(pdf_document, page_num)
                images.append(image)
...
def main():
    ...
    if uploaded_file:
                    ...
                    with st.expander(f"📄 {range_label}: Pages {start_pg}-{end_pg}", expanded=True):
                        ...
                        for page_num, page_text in page_data_list:
                            ...
                            if page_idx < len(page_images):
                                with col_img:
                                    # Resize image for better Word compatibility
                                    resized_img = extractor.resize_image_for_display(page_images[page_idx], max_width=600)
        ...
        with st.expander("ℹ️ How it works"):
            ...
            - ✅ Images sized to fit Word documents (600px width)
            - ✅ Individual downloads per range
            - ✅ Combined download for all ranges
            - ✅ Clean, readable output format
            - ✅ Uses PyMuPDF (no poppler dependency needed!)
            ...
            ### Installation:
            ```bash
            pip install streamlit pymupdf pillow requests pandas
            ```
            """)
